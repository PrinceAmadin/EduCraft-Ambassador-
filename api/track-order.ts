...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    // IMPORTANT: fetch from profile: (only set after admin approval)
    ...
    if (!profileStr) {
      ...
      if (!emailTo)                           { emailReason = "no_email"; }
        ...
        emailSent = await send(emailTo, "🎉 You just earned a referral commission! — EduCraft", commissionHtml(profile.name, jobDesc.trim(), amount, pct, commission));
...
function commissionHtml(name: string, jobDesc: string, amount: number, pct: number, commission: number) {
  const jobBlock = jobDesc ? `<div style="background:#FFF9ED;border-left:4px solid #fbdb21;padding:14px 18px;margin:18px 0;color:#0D5753;font-size:.88rem;line-height:1.6"><strong>Job Details:</strong><br/>${jobDesc}</div>` : "";
  const commBlock = amount > 0
    ? `<div style="background:#FFF9ED;border-radius:12px;padding:20px;margin:20px 0;text-align:center"><div style="font-size:.72rem;color:#12827c;font-weight:700;letter-spacing:.06em;margin-bottom:8px">YOUR COMMISSION</div><div style="font-size:2.2rem;font-weight:900;color:#0D5753">₦${commission.toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><div style="font-size:.82rem;color:#888;margin-top:6px">${pct}% of ₦${amount.toLocaleString("en-NG")}</div></div>`
    : `<div style="background:#FFF9ED;border-radius:12px;padding:16px;margin:20px 0;text-align:center;color:#12827c;font-weight:700">Commission Earned ✓ — EduCraft will confirm your exact amount shortly.</div>`;
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/></head><body style="font-family:'Segoe UI',sans-serif;background:#FFF9ED;margin:0;padding:32px 16px"><div style="max-width:480px;margin:0 auto;background:#fff;border:2px solid #E0B846;border-radius:16px;padding:40px 36px"><div style="text-align:center;margin-bottom:24px"><div style="font-size:2.8rem">🎉</div><h1 style="color:#0D5753;font-size:1.4rem;font-weight:800;margin:12px 0 0">You earned a referral!</h1></div><p style="color:#0D5753;line-height:1.7;margin:0 0 12px">Hi <strong>${name}</strong>,</p><p style="color:#0D5753;line-height:1.7;margin:0 0 4px">Your EduCraft referral link just brought in a confirmed new order!</p>${jobBlock}${commBlock}<p style="color:#0D5753;line-height:1.7;margin:0">Keep sharing your link — the more clients you refer, the more you earn! 🚀</p><div style="margin-top:28px;padding-top:18px;border-top:1px solid #E0B846;text-align:center;font-size:.72rem;color:#12827c;font-weight:700;letter-spacing:.08em">EDUCRAFT — Academic &amp; Technical Documentation Experts</div></div></body></html>`;
}
