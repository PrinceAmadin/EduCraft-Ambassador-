...
// Blocks if slot is already approved OR already pending.
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (await client.sIsMember("approved_ids", id)) {
      ...
      res.status(409).json({ error: "This Slot ID is already registered and active. If you think this is wrong, contact EduCraft." });
      return;
    }
