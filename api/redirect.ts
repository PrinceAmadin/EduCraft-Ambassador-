...
// ─── Ambassador Data ───────────────────────────────────────────────────────────
// ✏️ EDIT THIS to manage slots — keep in sync with src/ambassadors.ts
...
export default function handler(req: VercelRequest, res: VercelResponse): void {
  ...
  if (!id) {
    res.status(400).send(page("❌ Invalid Link", "No slot ID provided. Use a valid ambassador link.", "#ef4444"));
    return;
  }
  ...
  if (!slot) {
    res.status(404).send(page("❌ Not Found", `This ambassador link does not exist. Please contact EduCraft.`, "#ef4444"));
    return;
  }
  ...
  if (slot.status === "vacant") {
    ...
    res.status(410).send(page(`⚠️ Inactive Link`, `This ambassador slot is currently inactive. Please contact EduCraft directly @ ${res
                  .redirect(302, `https://wa.me/${EDUCRAFT_WHATSAPP}?text=${message}`)}.`, "#E0B846"));
    return;
  }

  // ✅ Use ambassador's real NAME in the WhatsApp message
  const message = encodeURIComponent(
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    h1{font-size:1.3rem;margin-bottom:12px;color:${accent}}
